...
function getExtraMysticalHand(g: Game, ofPlayerIdx: number, asSeenByPlayerIdx: number) {
  return (g.COMPAT_getMaskedTurnState(g.players[asSeenByPlayerIdx].id).maskedPlayerViews[ofPlayerIdx] as any)
...
describe('A three-player game', () => {
  ...
  it('After p0 gets X5, p0 and p1 get different results', () => {
    ...
    expect(
      (g.COMPAT_getMaskedTurnState(g.players[1].id).maskedPlayerViews[0].hand as TCardState[])
        .map(c => c.color + c.num)
        .join(','),
  ...
  it('p0 gets hinted of their X5, then sees that p2 knows about their A5', () => {
    ...
    expect(g.COMPAT_getMaskedTurnState(g.players[0].id).maskedPlayerViews[0].hand[4]).toEqual({
      actionability: 'UNDISCARDABLE',
      color: 'X',
      num: 5,
      hints: jasmine.any(Array),
